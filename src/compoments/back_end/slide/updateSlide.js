...
const UpdateSlide = () => {
    ...
    useEffect(()=>{
        const trademark =  async () => {
          try {
            const {data:slie} = await slideApi.get(id);
            setTra(slie);
          } catch (error) {
                console.log(error)
          }
    ...
    const newTrademarks = async (data)=>{
        ...
        let updateSlide = new FormData();   
       
        if(data.name){
            updateSlide.append('name',data.name)
        }
        if(image){
            updateSlide.append('photo',image)
        }
        const {data:newTra} = await slideApi.update(id,updateSlide)
        if(newTra){
           history.goBack();
        }
    }
